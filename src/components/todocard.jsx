...
export function TodoCard(props) {
  ...
  const [allChecklists, setAllChecklists] = useState(() => {
    return props.allTodos.reduce((obj, item) => {
      obj[item._id] = { checklist: item.checklist, title: item.title };
      return obj;
    }, {});
  });
  ...
  useEffect(()=>{setInitialMount(false)},[modalOpen]);
              ...
              <div className="flex flex-wrap mt-1 w-full items-center justify-start gap-2">
                ...
                {props.attachments.length != 0 && (
                  <div className="flex items-center gap-">
                    ...
                    <div>{props.attachments.length}</div>
                  </div>
                )}
                {allChecklists[props.id].checklist.length != 0 &&
